...
| `build-workdir.bats` | What survives between builds under `build-linux/`: `scripts/build-linux.sh` step 2 keeps `downloads/` and clears every other output; `build.sh`'s `sync_stage_to_dist` mirrors the staged tree into the Electron dist exactly (stale files go, Electron's `default_app.asar` stays), with rsync and with the `cp` fallback. Step 3 fails, naming the patch, when a main-bundle patch misses its anchor (the real helper-resolver assertion on a junk bundle) and when there is no bundle to patch. |
