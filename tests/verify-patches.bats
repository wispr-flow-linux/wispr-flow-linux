...
#
# verify-patches.sh greps a fixed set of markers (10 fixed strings + 1 Perl
...
@test "verify: exits 1 when the screenpos marker is missing" {
	local fixture
	fixture="$(write_fixture screenpos)"
	run "$VERIFY_SH" "$fixture"
	[[ "$status" -eq 1 ]]
	[[ "$output" == *'MISSING'* ]]
}
