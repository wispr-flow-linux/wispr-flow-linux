...
| `~/.config/autostart/wispr-flow.desktop` | The "Open at login" entry, present while the setting is on. Its `Exec=` passes `--hidden`, so a login start keeps the Hub closed. Your desktop's startup-apps settings can disable it. |
