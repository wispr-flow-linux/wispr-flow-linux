...
`build-linux/downloads/` is also kept across builds: the verified installer,
the Electron zip and the unpacked Electron runtime live there, and each is
re-verified or version-checked before reuse. Everything else under
`build-linux/` (the staged tree, the unpacked asar, the package outputs) is
regenerated by every build, so copy a package out before building the next
format.
